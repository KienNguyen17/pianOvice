...
public class Keyboard {
    ...
    public Keyboard(CanvasWindow canvas) {
        ...
        double x = 0;
        double y = canvas.getHeight() - whiteKeySize * 3.5;
        for (int i = 0; i < NUMBER_OF_WHITE_KEY; i++) {
            Key key = new Key(x, y, whiteKeySize, whiteKeySize * 3.5, Color.WHITE);
            canvas.add(key);
            keyList.add(key);
            x += whiteKeySize;
        } 
        ...
        double x2 = 0;
        double y2 = canvas.getHeight() - whiteKeySize * 3.5;
        List<Integer> num = List.of(1, 3, 6, 8, 10, 13, 15, 18, 20, 22);
        for (int i = 0; i < NUMBER_OF_BLACK_KEY; i++) {
            if (num.contains(i)) {
                Key key = new Key(x2, y2, blackKeySize, blackKeySize * 3.5, Color.BLACK);
                canvas.add(key);
                keyList.add(key);
            }
            x2 += blackKeySize;
